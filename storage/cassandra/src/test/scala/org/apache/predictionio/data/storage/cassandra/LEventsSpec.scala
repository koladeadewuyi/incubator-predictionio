...
class LEventsSpec extends Specification with TestEvents {
  ...
  PredictionIO Storage LEvents Specification
    ...
    - CassandraLEvents ${cassandraEvents}

  """

  def cassandraEvents = sequential ^ s2"""

    CassandraLEvents should
    - behave like any LEvents implementation ${events(cassandraDO)}
  ...
  def cassandraDO = Storage.getDataObject[LEvents](

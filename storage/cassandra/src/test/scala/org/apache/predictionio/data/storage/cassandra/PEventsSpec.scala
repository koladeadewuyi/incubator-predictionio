...
class PEventsSpec extends Specification with TestEvents {
  ...
  def cassandraLocal = Storage.getDataObject[LEvents](
    StorageTestUtils.cassandraSourceName,
    dbName
  )

  def cassandraPar = Storage.getDataObject[PEvents](
    StorageTestUtils.cassandraSourceName,
    dbName
  )
  ...
  PredictionIO Storage PEvents Specification
    ...
    - CassandraPEvents ${cassandraPEvents}
    - (stop Spark) ${Step(sc.stop())}

  """

  def cassandraPEvents = sequential ^ s2"""

    CassandraPEvents should
    - behave like any PEvents implementation ${events(cassandraLocal, cassandraPar)}
    - (table cleanup) ${Step(StorageTestUtils.dropCassandraNamespace(dbName))}
